...
class ClsEvaluator(HookBase):
    ...
    def eval(self):
        ...
        self.trainer.logger.info(
            "验证结果: mIoU/mAcc/allAcc {:.4f}/{:.4f}/{:.4f}.".format(
                m_iou, m_acc, all_acc
            )
        )
        for i in range(self.trainer.cfg.data.num_classes):
            self.trainer.logger.info(
                "类别_{idx}-{name} 结果: IoU/准确率 {iou:.4f}/{accuracy:.4f}".format(
                    idx=i,
    ...
    def after_train(self):
        self.trainer.logger.info(
            "最佳 {}: {:.4f}".format("allAcc", self.trainer.best_metric_value)
        )
...
class SemSegEvaluator(HookBase):
    ...
    def after_train(self):
        self.trainer.logger.info(
            "最佳 {}: {:.4f}".format("mIoU", self.trainer.best_metric_value)
        )
...
class InsSegEvaluator(HookBase):
    ...
    def eval(self):
        ...
        self.trainer.logger.info(
            "验证结果: mAP/AP50/AP25 {:.4f}/{:.4f}/{:.4f}.".format(
        ...
        for i, label_name in enumerate(self.valid_class_names):
            ...
            self.trainer.logger.info(
                "类别_{idx}-{name} 结果: AP/AP50/AP25 {AP:.4f}/{AP50:.4f}/{AP25:.4f}".format(
