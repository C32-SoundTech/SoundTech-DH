...
class IterationTimer(HookBase):
    ...
    def after_step(self):
        ...
        if "iter_info" in self.trainer.comm_info.keys():
            info = (
                "数据 {data_time_val:.3f} ({data_time_avg:.3f}) "
                "批次 {batch_time_val:.3f} ({batch_time_avg:.3f}) "
                "剩余 {remain_time} ".format(
...
class InformationWriter(HookBase):
    ...
    def before_step(self):
        ...
        # )
        info = "训练: [{epoch}/{max_epoch}][{iter}/{max_iter}] ".format(
    ...
    def after_step(self):
        ...
        self.trainer.comm_info["iter_info"] += "学习率: {lr:.5f}".format(lr=lr)
...
class CheckpointSaver(HookBase):
    ...
    def after_epoch(self):
        if is_main_process():
            ...
            self.trainer.logger.info("正在保存检查点到: " + filename)
            torch.save(
                {
                    "epoch": self.trainer.epoch + 1,
...
class CheckpointLoader(HookBase):
    ...
    def before_train(self):
        self.trainer.logger.info("=> 正在加载检查点与权重 ...")
        if self.trainer.cfg.weight and os.path.isfile(self.trainer.cfg.weight):
            self.trainer.logger.info(f"正在加载权重路径: {self.trainer.cfg.weight}")
            checkpoint = torch.load(
                self.trainer.cfg.weight,
                map_location=lambda storage, loc: storage.cuda(),
            )
            self.trainer.logger.info(
                f"按关键字加载层权重: {self.keywords}, "
                f"将关键字替换为: {self.replacement}"
            )
            weight = OrderedDict()
            ...
            self.trainer.logger.info(f"缺失的键: {load_state_info[0]}")
            if self.trainer.cfg.resume:
                self.trainer.logger.info(
                    f"在评估轮次恢复训练: {checkpoint['epoch']}"
                )
            ...
            self.trainer.logger.info(f"未在此路径找到权重: {self.trainer.cfg.weight}")
...
class PreciseEvaluator(HookBase):
    ...
    def after_train(self):
        ...
        if self.test_last:
            self.trainer.logger.info("=> 正在测试 model_last ...")
        else:
            self.trainer.logger.info("=> 正在测试 model_best ...")
...
class DataCacheOperator(HookBase):
    ...
    def before_train(self):
        self.trainer.logger.info(
            f"=> 正在缓存数据集: {self.data_root}, 划分: {self.split} ..."
        )
        if is_main_process():
...
class RuntimeProfiler(HookBase):
    ...
    def before_train(self):
        self.trainer.logger.info("正在进行运行时性能分析 ...")
        ...
        for i, input_dict in enumerate(self.trainer.train_loader):
            ...
            if self.backward:
                with profile(
                    ...
                    with record_function("model_inference"):
                        loss.backward()
            self.trainer.logger.info(f"性能分析: [{i + 1}/{self.warm_up + 1}]")
        if self.forward:
            self.trainer.logger.info(
                "前向性能分析: \n"
                + str(
...
class RuntimeProfilerV2(HookBase):
    ...
    def before_train(self):
        ...
        for i, input_dict in enumerate(self.trainer.train_loader):
            ...
            prof.step()
            self.trainer.logger.info(
                f"性能分析: [{i + 1}/{(self.wait + self.warmup + self.active) * self.repeat}]"
            )
        self.trainer.logger.info(
            "性能分析: \n"
            + str(
